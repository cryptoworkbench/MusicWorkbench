...
struct INTERVAL_sequence *initialize_MAJOR_SCALE() {
    struct INTERVAL_sequence *I_link = malloc(sizeof(struct INTERVAL_sequence)); I_link->content = whole_step; INTERVAL_sequence *I_link_backup = I_link;
    I_link->next = malloc(sizeof(struct INTERVAL_sequence)); I_link = I_link->next; I_link->content = whole_step;
    I_link->next = malloc(sizeof(struct INTERVAL_sequence)); I_link = I_link->next; I_link->content = half_step;
    I_link->next = malloc(sizeof(struct INTERVAL_sequence)); I_link = I_link->next; I_link->content = whole_step;
    I_link->next = malloc(sizeof(struct INTERVAL_sequence)); I_link = I_link->next; I_link->content = whole_step;
    I_link->next = I_link_backup; I_link = I_link->next;
    return I_link;
}

void initialize_ALL(struct NOTE_sequence **register0, struct INTERVAL_sequence **register1) {
    *register0 = initialize_CHROMATIC_SCALE();
    *register1 = initialize_MAJOR_SCALE();
}
...
// ^^^-- Function to print INTERVAL names
